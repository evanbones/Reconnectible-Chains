...
public class ChainRaycastHelper {
    ...
    public static boolean tryPlaceBunting(Player player, InteractionHand hand) {
        ...
        player.level().playSound(null, player.getX(), player.getY(), player.getZ(),
                SoundEvents.WOOL_PLACE, SoundSource.PLAYERS, 1.0f, 0.9f + player.level().getRandom().nextFloat() * 0.2f);
        return true;
    }
    ...
    public static boolean tryRemoveDecoration(Player player, InteractionHand hand) {
        ...
        if (bestEntry instanceof Chainable.ChainData.BuntingEntry buntingEntry) {
            ...
            if (!player.isCreative()) {
                Item item = BuiltInRegistries.ITEM.getValue(Identifier.fromNamespaceAndPath("supplementaries", "bunting_" + buntingEntry.color().getName()));
            ...
            if (!player.isCreative()) {
                Item bannerItem = BuiltInRegistries.ITEM.getValue(Identifier.withDefaultNamespace(bannerEntry.color().getName() + "_banner"));
            ...
            if (!player.isCreative()) {
                Item item = BuiltInRegistries.ITEM.getValue(hangingEntry.blockId());
                if (item != Items.AIR) player.getInventory().add(new ItemStack(item));
            }
            sendHangingSync(bestChainedEntity, bestHolder, bestLink, (ServerLevel) player.level());
        }
        player.level().playSound(null, player.getX(), player.getY(), player.getZ(),
                SoundEvents.ITEM_PICKUP, SoundSource.PLAYERS, 0.6f, 0.9f + player.level().getRandom().nextFloat() * 0.3f);
        return true;
    }
    ...
    public static boolean tryPlaceBanner(Player player, InteractionHand hand) {
        ...
        player.level().playSound(null, player.getX(), player.getY(), player.getZ(),
                SoundEvents.CHAIN_PLACE, SoundSource.PLAYERS, 1.0f, 0.9f + player.level().getRandom().nextFloat() * 0.2f);
        return true;
    }
    ...
    public static boolean tryPlaceHanging(Player player, InteractionHand hand) {
        ...
        Identifier blockId = BuiltInRegistries.ITEM.getKey(stack.getItem());
